 ...
 *
 *      Modifications by Peter Hanappe, Sony CSL
 *      Modifications by Anthony Beurive
 *
...
static int open_device(char * device_name, int * dev_fd)
{
	...
	if (ret < 0) {
		perr("Could not open %s", device_name);
		goto Done;
	}
	*dev_fd = ret;
	ret = 0;
...
static int packet_sync(struct wu_packet * p)
{
        // Look for '#'.
        int found_sharp = 0;
        size_t i = 0;
        ...
        // Move bytes so that '#' becomes the first.
        ...
        if (!found_semi_colon) return 0;

        return 1;
}

static int packet_parse(struct wu_packet * p)
{
	char * s, *next;
	int i;
...
static void packet_skip(struct wu_packet * p)
{
        char* s = strchr(p->buf, '#');
        if (s == NULL) {
                p->len = 0;
        ...
        // Keep the other bytes (past the end of the packet).
        size_t offset = s - p->buf;
        memmove(p->buf, p->buf + offset, p->len - offset);
        p->len -= offset;
}

static int wu_read(int fd, struct wu_packet * p)
{
        // Static packet to keep all read bytes.  FIXME: works with
        // only one device.
        static struct wu_packet _p = { .len = 0 };
	...
	struct timeval tv;
        ...
        while (1) {
                ...
                int ret = select(fd + 1, &read_fd, NULL, NULL, &tv);
                if (ret < 0) {
                        ...
                        size_t max_read = sizeof(_p.buf) - 1 - _p.len;
                        int ret = read(fd, _p.buf + _p.len, max_read);
                        if (ret < 0) {
                ...
                if (packet_sync(&_p)) {
                        memcpy(p->buf, _p.buf, _p.len);
                        p->len = _p.len;
                        int err = packet_parse(p);
                        packet_skip(&_p);
                        return err;
                }
        }
...
static int wu_start_log(int fd)
{
	struct wu_packet p = {
		.cmd		= 'L',
		.sub_cmd	= 'W',
...
int wu_open()
{
	int ret = open_device(wu_device, &wu_fd);
	if (ret) {
                wu_fd = -1;
		return ret;
        }
...
double wu_watts()
{
	struct wu_packet p = {
        ...
        int ret = wu_read(wu_fd, &p);
        if (ret)
                return -1.0;

	//print_packet(&p, "Debug");

        return strtod(p.field[0], NULL) / 10.0;
}
